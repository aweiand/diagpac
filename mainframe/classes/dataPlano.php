...
class dataPlano{
    ...
    function seed($tab, $campo) {
    	...
    	$ret = $ret+1;
    	$sql = "ALTER TABLE planestrat.". $tab ." AUTO_INCREMENT = ". $ret;
    	$alt = $this->query($sql);
    	return $ret;
    }
    ...
    /**
     * Funcao para recuperar os dados do aluno pelo nome
     * @access public
     * @param String $nome - Nome do Aluno ou NULL para todos
     * @return Recordset - com dados da Consulta Realizada
     */
    function getAluno($nome = ''){
        $cmdSQL = "SELECT firstname, lastname, email, id FROM moodle.moodle_user WHERE firstname LIKE '".$nome."%'
    ...
    /**
     * Funcao para recuperar os dados do aluno pelo e-mail
     * @access public
     * @param String $email - E-mail do Aluno ou NULL para todos
     * @return Recordset - com dados da Consulta Realizada
     */
    function getAlunoEmail($email = ''){
        $cmdSQL = "SELECT firstname, lastname, email, id FROM moodle_user WHERE email ='".$email."'
					AND suspended = 0 AND deleted = 0";
        $ret = $this->query($cmdSQL);
        return $ret;
    }
    ...
    /**
     * Funcao para recuperar os dados do aluno pelo ID
     * @access public
     * @param Integer $id - ID do Aluno ou NULL para todos
     * @return Recordset - com dados da Consulta Realizada
     */
    function getAlunoId($id = ''){
        $cmdSQL = "SELECT firstname, lastname, email, id FROM moodle_user WHERE id=".$id."
					AND suspended = 0 AND deleted = 0";
        $ret = $this->query($cmdSQL);
        return $ret;
    }    
